...
class Ant(pg.sprite.Sprite):
    def __init__(self, drawSurf, nest, pheroLayer, environment, index):
        super().__init__()
    ...
    def update(self, dt):  # behavior
        ...
        # エサ探索時の移動方向の決定
            ...
            if mid_result[1] > max(left_result[1], right_result[1]):
                ...
                wandrStr = .1
            # ネガティブフェロモンへの応答
            elif self.pos.distance_to(self.nest) >= 96 and left_result[1] == right_result[1] == mid_result[1] == 0:
                if mid_result[2] < min(left_result[2], right_result[2]):
                    self.desireDir += pg.Vector2(1,0).rotate(self.ang).normalize()
                    wandrStr = .1
                elif left_result[2] < right_result[2]:
                    self.desireDir += pg.Vector2(1,-2).rotate(self.ang).normalize()
                elif right_result[2] < left_result[2]:
                    self.desireDir += pg.Vector2(1,2).rotate(self.ang).normalize()
        ...
        # エサ保有時の移動方向の決定
            ...
            if self.pos.distance_to(self.nest) < 24:
                ...
                self.environment.total_food += 1
                if self.environment.ants_condition[self.index][2] > 300:
                    self.environment.ants_condition[self.index][1] = 0
                    self.environment.ants_condition[self.index][2] = 0
                    self.environment.ants_condition[self.index][3] = self.environment.total_food # これは任意
                    self.environment.workers.remove(self)
                    self.environment.ants_number[2] -= 1
                    self.environment.ants_number[0] += 1
...
class PheroGrid():
    ...
    def update(self, dt):
        ...
        # 2番目の要素（緑成分）のみを更新
        self.img_array[:, :, 1] -= 0.1 * (60/FPS) * ((dt/10) * FPS)
        # 3番目の要素（青成分）のみを更新
        self.img_array[:, :, 2] -= 0.4 * (60/FPS) * ((dt/10) * FPS)
...
class Environment():
    def __init__(self, workers):
        ...
        self.ants_condition = [[100, 1, 0, 0] for _ in range(ANTS)]
        for i in range(int(ANTS*0.8//1)):
            self.ants_condition[i][0] = 30
        for i in range(int(ANTS*0.2//1)):
            self.ants_condition[i][0] = 1
        # self.ants_condition = [[random.randint(0, 100), 1, 0, 0] for _ in range(ANTS)]
        self.ants_number = [0, ANTS, 0] # 休憩、待機、出勤

def main():
    # csvを読み込み、ヘッダーを記入
    with open('ants.csv', 'w') as f:
        writer = csv.writer(f)
        writer.writerow(['time', 'food', 'rest', 'wait', 'work'])
    ...
    # for n in range(ANTS):
    #     workers.add(Ant(screen, nest, pheroLayer, environment))
    ...
    #     dist = pow(i / (foodBits - 1.0), 0.5) * fRadius
    #     angle = 2 * pi * 0.618033 * i
    #     fx = cur_w*1/3 + dist * cos(angle)
    #     fy = cur_h*7/8 + dist * sin(angle)
    #     foods.add(Food((fx,fy)))
    # foodList.extend(foods.sprites())
    ...
    while True:
        ...
        pheroLayer.img_array[114:126,40:120] = (50,50,50)  # wall

        # アリの状態のアップデート！
        workers.update(dt)

        # 各アリの勤怠状態更新からの時間経過を記録
        for ant in environment.ants_condition:
            ant[2] += dt

        for index, ant in enumerate(environment.ants_condition):
            # 待機中
            if ant[1] == 1:
                # n秒経った
                if ant[2] > ant[0]:
                    # 餌が増えてない
                    if environment.total_food <= ant[3]:
                        # 出勤
                        workers.add(Ant(screen, nest, pheroLayer, environment, index))
                        environment.ants_number[1] -= 1
                        environment.ants_number[2] += 1
                        ant[1] = 2
                        ant[2] = 0
                        ant[3] = environment.total_food # これは任意
                    # 餌が増えた
                    else:
                        # カウントリセット
                        ant[2] = 0
                        ant[3] = environment.total_food
                else:
                    if environment.total_food > ant[3]:
                        # カウントリセット
                        ant[2] = 0
        ...
        screen.blit(font.render("food : "+str(int(environment.total_food)), True, [0,200,0]), (8, 40))
        screen.blit(font.render("time : "+str(round(float(time)/10, 1)), True, [0,200,0]), (8, 80))
        screen.blit(font.render("rest : "+str(environment.ants_number[0]), True, [0,200,0]), (8, 120))
        screen.blit(font.render("wait : "+str(environment.ants_number[1]), True, [0,200,0]), (8, 160))
        screen.blit(font.render("work : "+str(environment.ants_number[2]), True, [0,200,0]), (8, 200))

        pg.display.update()

        # csvに記録
        if time // 10 > time_b // 10:
            # print("here")
            time_b = time
            with open('ants_14.csv', 'a') as f:
                writer = csv.writer(f)
                writer.writerow([time//10, environment.total_food, environment.ants_number[0], environment.ants_number[1], environment.ants_number[2]])  
